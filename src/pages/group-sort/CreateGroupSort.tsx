...
function CreateGroupSort() {
  ...
  const removeCategory = (categoryIndex: number) => {
    if (categories.length <= 2) {
      toast.error("Minimal 2 kategori diperlukan");
      return;
    }
  ...
  const removeItem = (categoryIndex: number, itemIndex: number) => {
    ...
    if (updated[categoryIndex].items.length <= 1) {
      toast.error("Setiap kategori harus memiliki minimal 1 item");
      return;
    }
  ...
  const validateForm = () => {
    ...
    if (timeLimit > 600) {
      showErrorToast("Waktu limit maksimal 600 detik (10 menit)");
      return false;
    }
    ...
    if (categories.length > 10) {
      showErrorToast("Maksimal 10 kategori yang diizinkan");
      return false;
    }
    for (let i = 0; i < categories.length; i++) {
      ...
      if (category.items.length > 20) {
        showErrorToast(`Kategori "${category.name}" maksimal 20 item`);
        return false;
      }
    ...
    if (duplicateCategoryNames.length > 0) {
      showErrorToast(
        `Nama kategori tidak boleh sama: "${duplicateCategoryNames[0]}"`,
      );
      return false;
    }
    ...
    for (const category of categories) {
      for (const item of category.items) {
        ...
        if (allItemTexts.includes(itemText)) {
          showErrorToast(
            `Item dengan teks "${item.text}" sudah ada di kategori lain`,
          );
          return false;
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("time_limit", (timeLimit || 30).toString());
      formData.append("score_per_item", (scorePerItem || 10).toString());
      formData.append(
      ...
      let userFriendlyMsg = "Gagal membuat game. Silakan coba lagi.";
      if (errorMsg) {
        if (errorMsg.includes("time_limit")) {
          userFriendlyMsg = "Waktu limit tidak valid. Harus 30-600 detik.";
                ...
                <div className="relative">
                  ...
                  <div className="absolute right-1 top-1/2 -translate-y-1/2 flex flex-col">
                    <button
                      type="button"
                      onClick={() =>
                        setTimeLimit((prev) => Math.min(600, (prev || 30) + 1))
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setTimeLimit((prev) => Math.max(30, (prev || 30) - 1))
                      }
