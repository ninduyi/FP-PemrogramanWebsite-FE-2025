...
export default function EditGroupSort() {
    ...
    return (
      <div className="w-full space-y-3">
        ...
        >
          {!preview && (
            <FileUpload.Dropzone
              className={`p-6 border-2 border-dashed border-purple-500/50 rounded-xl flex flex-col items-center text-center gap-3 bg-gray-900/30 hover:border-cyan-400/50 hover:bg-gray-900/40 transition-all duration-300 ${
                showPreview ? "" : "p-3"
              }`}
            >
              <Upload
          ...
          {preview && (
            <div
              className={`flex items-center gap-3 p-3 border border-purple-500/30 rounded-lg bg-gray-900/30 backdrop-blur-sm ${
                showPreview ? "" : ""
              }`}
            >
              <div
                className={`${
                  showPreview ? "size-16" : "size-12"
                } rounded-lg overflow-hidden border-2 border-cyan-400/50 flex-shrink-0`}
              >
                <img
                  src={preview}
                  alt="Preview"
              ...
              <div
                className={`flex-1 text-sm truncate text-cyan-300 font-mono ${
                  showPreview ? "text-sm" : "text-xs"
                }`}
              >
  ...
  const handleDeleteCategory = (categoryId: string) => {
    if (categories.length <= 2) {
      showErrorToast("Minimal 2 kategori diperlukan");
      return;
    }
  ...
  const validateForm = () => {
    if (!name.trim()) {
      showErrorToast("Nama game harus diisi");
      return false;
    }

    if (!description.trim()) {
      showErrorToast("Deskripsi game harus diisi");
      return false;
    }

    // Validasi time limit
    if (!timeLimit || timeLimit < 30) {
      showErrorToast("Waktu limit minimal 30 detik");
      return false;
    }
    if (timeLimit > 3600) {
      showErrorToast("Waktu limit maksimal 3600 detik (1 jam)");
      return false;
    }

    // Validasi score per item
    if (!scorePerItem || scorePerItem < 10) {
      showErrorToast("Poin per item minimal 10");
      return false;
    }
    if (scorePerItem > 1000) {
      showErrorToast("Poin per item maksimal 1000");
      return false;
    }
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      if (!category.name.trim()) {
        showErrorToast(`Kategori ${i + 1} harus memiliki nama`);
        return false;
      }

      if (category.items.length === 0) {
        showErrorToast(
          `Kategori "${category.name}" harus memiliki minimal 1 item`,
        );
        return false;
      }
      ...
      for (let j = 0; j < category.items.length; j++) {
        ...
        if (!item.text.trim()) {
          showErrorToast(
            `Item ${j + 1} di kategori "${category.name}" harus memiliki teks`,
          );
          return false;
        }
        if (!item.hint || !item.hint.trim()) {
          showErrorToast(
            `Item ${j + 1} di kategori "${category.name}" harus memiliki petunjuk (hint)`,
          );
          return false;
        }
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formDataToSend.append("score_per_item", String(scorePerItem));
      formDataToSend.append("time_limit", String(timeLimit));
      formDataToSend.append(
        "is_category_randomized",
        String(isCategoryRandomized),
      );
      formDataToSend.append("is_item_randomized", String(isItemRandomized));
      formDataToSend.append(
        "categories",
      ...
      let userFriendlyMsg = "Gagal menyimpan game. Silakan coba lagi.";
      if (errorMsg) {
        if (errorMsg.includes("time_limit")) {
          userFriendlyMsg = "Waktu limit tidak valid. Harus 30-3600 detik.";
        } else if (errorMsg.includes("score_per_item")) {
          userFriendlyMsg = "Poin per item tidak valid. Harus minimal 10.";
        } else if (errorMsg.includes("Blob")) {
          userFriendlyMsg =
            "Error saat upload file. Pastikan file format PNG/JPEG dan ukuran < 5MB.";
        } else if (errorMsg.includes("categories")) {
          userFriendlyMsg =
            "Error pada kategori. Pastikan minimal 2 kategori dan setiap kategori punya item.";
        } else {
                        ...
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="relative">
                            ...
                            <span className="text-red-400 text-xs font-mono mt-1">
                              * Hint wajib diisi untuk setiap item
                            </span>
                          </div>
                        </div>
