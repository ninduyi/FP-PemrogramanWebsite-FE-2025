...
function GroupSort() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      ...
      try {
        ...
        // Check if user is logged in (optional: check localStorage for auth token)
        const token =
        ...
        try {
          ...
          // If public fails (404 - unpublished) AND user is logged in, try private endpoint for testing
          if (
            userIsLoggedIn &&
        ...
        gameData.game_data.categories.forEach((cat: Category) => {
          cat.items.forEach(
            (item: {
              ...
              // Convert image path to proper format for persistence
              ...
              if (
                ...
                // If it's a relative path from backend, convert to full API URL
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Fetch highest score SETELAH submit (fail gracefully for guests)
      try {
        const highestScoreData = await ScoreAPI.getHighestScore(game!.id);
        ...
        if (
          ...
          // For guest or first-time player, use current score as best score
          setHighestScore(scoreToSubmit);
          console.log(
            "Guest user or first-time: best score set to current score:",
            scoreToSubmit,
          );
        }
      } catch (highestScoreErr) {
        console.warn(
          "Could not fetch highest score (guest user may not have history):",
          highestScoreErr,
        );
        // For guest, use current score as best score
        setHighestScore(scoreToSubmit);
      }

      // Fetch leaderboard (always available - guest or logged in)
      // Add retry logic since DB might not have synced immediately
      let leaderboardData = null;
      let retries = 0;
      const maxRetries = 3;

      // Get current user from auth store to check if logged in
      const authUser = useAuthStore.getState().user;
      const isUserLoggedIn = !!authUser?.id;

      while (retries < maxRetries && !leaderboardData) {
        try {
          const data = await ScoreAPI.getLeaderboard(game!.id, 5);
          console.log("Leaderboard data attempt", retries + 1, ":", data);

          if (Array.isArray(data) && data.length > 0) {
            leaderboardData = data;

            // If user is logged in, make sure their entry has correct username
            ...
            console.log("Leaderboard set with", data.length, "entries");
            break;
          } else {
            // If empty on first try, wait a bit and retry
            if (retries < maxRetries - 1) {
              console.log("Leaderboard empty, retrying...");
              await new Promise((resolve) => setTimeout(resolve, 500)); // Wait 500ms
            }
          }
        } catch (leaderboardErr) {
          console.warn(
            "Leaderboard fetch attempt",
            retries + 1,
            "failed:",
            leaderboardErr,
          );
      ...
      // If still no leaderboard data after retries
      if (!leaderboardData || leaderboardData.length === 0) {
        console.log("Using fallback leaderboard");

        // For logged-in users, create entry with their name
        if (isUserLoggedIn && authUser) {
          const userEntry = {
            user_id: authUser.id,
            username: authUser.username,
            highest_score: scoreToSubmit,
            total_plays: 1,
          };
          setLeaderboard([userEntry] as unknown as typeof leaderboard);
          console.log(
            "Created fallback entry for logged-in user:",
            authUser.username,
          );
        } else {
          // For guests, create guest entry
          const guestEntry = {
            user_id: "guest",
          ...
          setLeaderboard([guestEntry] as unknown as typeof leaderboard);
          console.log("Created fallback entry for guest");
        }
      }
    } catch (err: unknown) {
